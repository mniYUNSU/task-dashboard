...
- コンポーネント設計と再利用（TaskCard を /tasks・/list・ダッシュボードで共通利用）
- ドメインロジックの分離（進捗計算・期限判定）
- UI の一貫性（Dialog / Toast / Badge の統一）
- モバイルファーストのレスポンシブ設計
- Hydration mismatch を防ぐための設計（ThemeProvider とクライアント判定の導入）
- 可読性・保守性を意識した構成
...
| 作業内容                                    | 時間       |
| ------------------------------------------- | ---------- |
| プロジェクト初期設定・設計                  | 1.5 時間   |
| 基本機能実装（タスク管理）                  | 3.0 時間   |
| ダッシュボード実装                          | 1.5 時間   |
| UI/UX 改善（Dialog / Toast / レスポンシブ） | 2.0 時間   |
| 追加機能実装（期限管理など）                | 2.0 時間   |
| デバッグ・調整                              | 1.5 時間   |
| README 作成                                 | 0.5 時間   |
| 合計                                        | 約 12 時間 |
...
- AI ツール（Codex）は実装の生産性支援・叩き台作成に活用しました。
- 要件解釈、設計判断、最終決定・確認は手動で行っています。
- 今後の改善候補: フィルタ条件の URL 共有、キーボード操作の拡充、エクスポート機能。
- アプリは Vercel にデプロイ済みです。公開 URL は https://task-dashboard-beta-ten.vercel.app です。ローカル実行も可能です。
